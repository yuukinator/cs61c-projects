...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x4: // opcode == 0x4 (BEQ)
    if (p->R[inst.itype.rs] == p->R[inst.itype.rt]) {
        p->pc = p->pc+4 + (int32_t) (int16_t) (inst.itype.imm << 2);
    }
    else {
	p->pc += 4;
    }
  ...
  case 0x9: // opcode == 0x9 (ADDIU)
    p->R[inst.itype.rt] = p->R[inst.itype.rs] + (uint32_t) (int32_t) (int16_t) inst.itype.imm;
    p->pc += 4;
    break;
  ...
  case 0x20: // opcode == 0x20 (LB)
    p->R[inst.itype.rt] = (int32_t) (int8_t) (load_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_BYTE));
    p->pc += 4;
    break;

  case 0x21: // opcode == 0x21 (LH)
    p->R[inst.itype.rt] = (int32_t) (int16_t) load_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_HALF_WORD);
    p->pc += 4;
    break;

  case 0x23: // opcode == 0x23 (LW)
    p->R[inst.itype.rt] = load_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_WORD);
    p->pc += 4;
    break;

  case 0x24: // opcode == 0x24 (LBU)
    p->R[inst.itype.rt] = load_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_BYTE);
    p->pc +=4;
    break;


  case 0x25: // opcode == 0x25 (LHU)
    p->R[inst.itype.rt] = load_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_HALF_WORD);
    p->pc += 4;
    break;

  case 0x28: // opcode == 0x28 (SB)
    store_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_BYTE, p->R[inst.itype.rt]);
    p->pc += 4;
    break;

  case 0x29: // opcode == 0x29 (SH)
    store_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_HALF_WORD, p->R[inst.itype.rt]);
    p->pc += 4;
    break;

  case 0x2b: // opcode == 0x2b (SW)
    store_mem((int32_t) p->R[inst.itype.rs] + (int32_t) (int16_t) inst.itype.imm, SIZE_WORD, p->R[inst.itype.rt]);
    p->pc += 4;
    break;
